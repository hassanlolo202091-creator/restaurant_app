...
from languages import get_ui_text, TRANSLATIONS
...
def add_menu_item(item_name, input_lang_code, price):
    names_dict = {}
    for target_lang_name, target_lang_code in LANGUAGES.items():
        if target_lang_code == input_lang_code:
            ...
            try:
                translated = GoogleTranslator(source=input_lang_code, target=target_lang_code).translate(item_name)
                names_dict[target_lang_code] = translated if translated else item_name
            except Exception:
                names_dict[target_lang_code] = item_name
...
# --- دالة إرجاع اسم الوجبة الذكية (تتعامل مع المنيو القديم والجديد) ---
def get_item_display_name(item, lang_code):
    # 1. إذا كان العنصر بالتنسيق الجديد (يحتوي قاموس أسماء للغات)
    if isinstance(item, dict) and "names" in item and isinstance(item["names"], dict):
        return item["names"].get(lang_code, item["names"].get("ar", ""))

    # 2. إذا كان بالتنسيق القديم (اسم عربي مباشر)
    raw_name = item.get("name", "") if isinstance(item, dict) else str(item)
    if lang_code == "ar" or not raw_name:
        return raw_name

    # البحث في القاموس الثابت
    lang_data = TRANSLATIONS.get(lang_code, {})
    items_dict = lang_data.get("items", {})
    if raw_name in items_dict:
        return items_dict[raw_name]

    # ترجمة فورية احتياطية
    try:
        translated = GoogleTranslator(source="ar", target=lang_code).translate(raw_name)
        if translated:
            return translated
    except Exception:
        pass
...
if admin_mode:
    ...
    if password == "1234":
        ...
        with tab_admin1:
            ...
            if orders:
                for idx, o in enumerate(reversed(orders), 1):
                    with st.expander(f"طلب #{idx} - العميل: {o.get('customer')} | الإجمالي: {o.get('total')} درهم"):
        ...
        with tab_admin2:
            ...
            if reservations:
                for r in reversed(reservations):
                    st.write(f"📌 **{r.get('name')}** - عدد الأفراد: {r.get('guests')} | التاريخ: {r.get('date')} | الوقت: {r.get('time')}")
                    st.markdown("---")
            else:
        ...
        with tab_admin3:
            ...
            with col_a:
                admin_input_lang = st.selectbox("لغة كتابة اسم الوجبة", list(LANGUAGES.keys()), index=0)
            ...
            if st.button("إضافة الوجبة وتوليد الترجمات آلياً"):
                if new_name:
                    ...
                    st.success(f"تمت إضافة {new_name} وترجمتها بنجاح!")
                    st.rerun()
                else:
            ...
            for item in current_menu:
                ...
                disp_name = get_item_display_name(item, lang_code)
...
# --- واجهة الزبون ---
    ...
    with tab1:
        ...
        for item in menu_items:
            display_name = get_item_display_name(item, lang_code)
            price = item.get("price", 0) if isinstance(item, dict) else 0
            item_id = item.get("id", display_name) if isinstance(item, dict) else display_name
            ...
            with c2:
                qty = st.number_input(
                    "الكمية", min_value=1, value=1, key=f"qty_{item_id}", label_visibility="collapsed"
                )

            with c3:
                if st.button(get_ui_text("add_item", lang_code), key=f"btn_{item_id}"):
    ...
    with tab2:
        ...
        if not st.session_state.cart:
            ...
            price_dict = {}
            for item in menu_items:
                disp = get_item_display_name(item, lang_code)
                p = item.get("price", 0) if isinstance(item, dict) else 0
                price_dict[disp] = p
            ...
            for item_disp_name, quantity in st.session_state.cart.items():
                ...
                st.write(f"**{item_disp_name}** × {quantity} = {item_total:.1f} {curr}")
