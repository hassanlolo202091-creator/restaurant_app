...
def translate_food_item(item_arabic, lang_code):
    ...
    # 2. الترجمة المباشرة عبر GoogleTranslator
    try:
        translated = GoogleTranslator(source='auto', target=lang_code).translate(item_arabic)
        if translated:
            return translated
    ...
    if password == "1234":
        ...
        with tab_admin3:
            ...
            for item in current_menu:
                ...
                with c1:
                    disp = translate_food_item(item['name'], lang_code)
                    st.write(f"• **{item['name']}** ({disp}) - {item['price']} درهم")
                with c2:
...
# --- واجهة العميل ---
    ...
    with tab1:
        ...
        for item in menu_items:
            ...
            with c2:
                qty = st.number_input(
                    "الكمية", min_value=1, value=1, key=f"qty_{item['id']}", label_visibility="collapsed"
                )

            with c3:
                if st.button(get_ui_text("add_item", lang_code), key=f"btn_{item['id']}"):
