...
# إضافة التخزين المؤقت لتسريع الترجمة لأقصى درجة
@st.cache_data
...
if admin_mode:
  if st.session_state.admin_language is None:
    ...
    for idx, (lang_name, lang_code) in enumerate(LANGUAGES.items()):
      with cols[idx]:
        if st.button(
            f"Admin: {lang_name}", use_container_width=True, type="primary"
        ):
          st.session_state.admin_language = lang_code
          st.rerun()

  else:
    admin_lang = st.session_state.admin_language

    if st.sidebar.button("Change Admin Language / تغيير لغة الأدمن"):
      st.session_state.admin_language = None
      st.rerun()
        ...
        if orders:
          for idx, o in enumerate(reversed(orders), 1):
            with st.expander(
                ...
                f" {translate_text('Customer', admin_lang)}:"
                f" {o.get('customer')} | {translate_text('Total', admin_lang)}:"
                f" {o.get('total')} {translate_text('AED', admin_lang)}"
            ):
              st.write(
      ...
      with tab2:
        ...
        if reservations:
          for r in reversed(reservations):
            st.write(
                f"📌 **{r.get('name')}** -"
                f" {translate_text('Guests Count', admin_lang)}:"
    ...
    with tab3:
      ...
      if st.button(reserve_btn_text):
        if res_name:
          ...
          st.warning(translate_text("Please enter reservation name", lang)) App", page_icon="🍔", layout="wide")
